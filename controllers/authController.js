...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    ...
    ), // the browser will delete the cookie after it expired
    secure: true,
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      //since we run the protect middleware before, we have acess to the req variable
      return next(
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(
      new AppError('The current password informed is incorrect!', 401),
    );
  }
